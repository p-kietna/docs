Ah, I see — the error you're getting from `queue.push(...failed)` likely means that `failed` is not an array, but a single object or `undefined`. TypeScript is strict about this.
